...
create_experiment <- function(
  ...
  base_experiment(experiment_name, project_name, workspace_name, api_key, keep_active, log_output,
                  log_error, log_code, log_system_details, log_git_info)
}
...
base_experiment = function(
  experiment_name = NULL, project_name = NULL, workspace_name = NULL, api_key = NULL,
  keep_active = TRUE, log_output = TRUE, log_error = FALSE,
  log_code = TRUE, log_system_details = TRUE, log_git_info = FALSE, experiment_key = NULL
) {
...
#' experiment. Use [`create_experiment()`] to create or [`get_experiment()`] to get a
...
Experiment <- R6::R6Class(
  ...
  public = list(
	...
	if (!isTRUE(.cometrenv$cancreate)) {
          comet_stop("Do not call this function directly. Use `create_experiment()` or `get_experiment()` instead.")
	}
	LOG_DEBUG("Creating experiment ", experiment_key)
	.cometrenv$cancreate <- FALSE
	.cometrenv$curexp <- self
      } else {
	LOG_DEBUG("Getting experiment ", experiment_key)
      }
    ...
    #' @description
    #' Get the archived status of an experiment.
    get_archived = function() {
      private$archived
    },
  ...
  private = list(
    ...
    check_active = function() {
      ...
      # Non-online experiments are always active
    },
